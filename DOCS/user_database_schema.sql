...
CREATE TABLE public.pagos_en_linea (
  ...
  medio_pago text NOT NULL DEFAULT 'Plin'::text,
  estado text NOT NULL DEFAULT 'completado'::text CHECK (estado = ANY (ARRAY['completado'::text, 'pendiente'::text, 'fallido'::text])),
...
CREATE TABLE public.plin_transacciones (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  tipo text NOT NULL CHECK (tipo = ANY (ARRAY['envio'::text, 'recepcion'::text, 'pago_linea'::text])),
  destinatario text,
  comercio text,
  monto numeric NOT NULL,
  estado text NOT NULL DEFAULT 'completado'::text CHECK (estado = ANY (ARRAY['completado'::text, 'pendiente'::text, 'fallido'::text])),
